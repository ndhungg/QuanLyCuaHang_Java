...
public class NhaCungCapControll {
    ...
    public boolean update(NhaCungCap ncc) throws Exception {
        ...
        try (Connection conn = ConnectSQL.getConnection();
            ...
            pstm.setString(4, ncc.getEmail());
            pstm.setString(5, ncc.getMaNhaCungCap());
            return pstm.executeUpdate() > 0;
        }
    }
    ...
    public List<NhaCungCap> findByName(String tenNCC) throws Exception {
        String sql = "Select *from NhaCungCap where TenNhaCungCap like N'%" + tenNCC + "%'";
        try (Connection conn = ConnectSQL.getConnection();
                PreparedStatement pstm = conn.prepareCall(sql);) {
            try (ResultSet rs = pstm.executeQuery()) {
                List<NhaCungCap> list = new ArrayList<>();
